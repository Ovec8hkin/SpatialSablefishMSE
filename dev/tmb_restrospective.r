...
peels <- seq(1, nyears-63, 5)
grid <- expand.grid(sims=1:nsims, peels=peels)
...
out <- pbapply::pblapply(1:nrow(grid), function(n, grid, sable_om, mse_tier3, nyears){
    library(tidyverse)
    library(TMB)
    ...
    assess_inputs <- format_em_data(
      ...
      added_years = i,
      file_suffix = paste0(s,"_",i)
    )
    file.remove(paste0("data/sablefish_em_data_curr_",s,"_",i,".RDS"))
    file.remove(paste0("data/sablefish_em_par_curr_",s,"_",i,".RDS"))
    
    #' Figure out true rec devs
    # bias_ramp <- rep(0, y)
    # for(x in (1960):(2022+y-64)){
    #     ii <- x - (1960) +1
    #     if(x < 1980){
    #         bias_ramp[ii] <- 0
    #     }else if(x >= 1980 & x < 1990){
    #         bias_ramp[ii] <- 1*(1-((x-1980)/(1990-1980)))
    #     }else if(x >= 1990 & x < (1960+y-5)){
    #         bias_ramp[ii] <- 1
    #     }else if(x >= (1960+y-5)){
    #         bias_ramp[ii] <- 1*(((x-2022+y)/(2022+y-(1960+y-5))))
    #     }
    # }

    # bias_ramp <- bias_ramp[(length(bias_ramp)-y+1):length(bias_ramp)]
    # sigrs <- c(rep(0.40, 15), rep(1.20, 5), rep(1.04, y-15-5))
    # recruitment <- apply(mse_tier3$naa[1:y,1,,,s], 1, sum)
    # ln_mean_rec <- mean(log(recruitment))
    # ln_rec_devs <- log(recruitment) - ln_mean_rec + bias_ramp*sigrs^2/2

    # fix_pars <- list(
    #   ln_mean_rec = ln_mean_rec,
    #   ln_rec_dev = ln_rec_devs
    # )
    ...
    om_rec <- apply(mse_tier3$naa[1:y,1,,,s], 1, sum)
    ...
    om_Fs <- mse_tier3$out_f[1:y,1, 1, 1, s]
    
    est_fish_Fs <- SpatialSablefishAssessment::get_fishing_mortalities(mle_report) %>% as_tibble() %>%
        mutate(
            true_Fs = c(apply(mse_tier3$faa[1:y,,,1,1,s], 1, max), apply(mse_tier3$faa[1:y,,,1,2,s], 1, max)),
            avg_F = c(rep(exp(mle_optim$par["ln_ll_F_avg"]), y), rep(exp(mle_optim$par["ln_trwl_F_avg"]), y)),
            true_avg_F = c(rep(exp(NA), y), rep(exp(NA), y))
        )
    ...
    joint_self_true <- apply(sable_om$dem_params$sel[y,,1,,,drop=FALSE], c(1, 2), sum)/max(apply(sable_om$dem_params$sel[y,,1,,,drop=FALSE], c(1, 2), sum))
    joint_self_est <- apply(sel_est, c(1, 2), sum)/max(apply(sel_est, c(1, 2), sum))
    rps_est <-  calculate_ref_points(30, mle_report$M[1,1], sable_om$dem_params$mat[y,,1,1], sable_om$dem_params$waa[y,,1,1], joint_self_est, rep(1, 30), avg_rec=mean(est_rec$Recruitment[17:y]/2))
    rps_true <- calculate_ref_points(30, sable_om$dem_params$mort[1,1,1,1], sable_om$dem_params$mat[y,,1,1], sable_om$dem_params$waa[y,,1,1], joint_self_true, rep(1, 30), avg_rec=mean(mse_tier3$naa[17:y, 1, 1, 1, s]))
    ...
    return(tmp)
}, grid=grid, sable_om=sable_om, mse_tier3=mse_tier3, nyears=nyears, cl=cl)
...
ggsave("~/Desktop/text.png")

#' Plot retrospective for referece points
data %>% 
  select(Year, F40_est, F40_true, F35_est, F35_true, B40_est, B40_true, sim, peel) %>%
  filter(peel %in% seq(1, 100, 10)) %>%
  group_by(Year, peel) %>%
  median_qi(F40_est, F40_true, F35_est, F35_true, B40_est, B40_true, .width=c(0.50, 0.95)) %>%
  reformat_ggdist_long(2) %>%
  separate(name, into=c("RP", "model"), sep="_") %>%

  ggplot()+
    geom_pointrange(aes(x=model, y=median, ymin=lower, ymax=upper, color=peel))+
...
#' Plot retrospective for SSB
data %>% as_tibble() %>%
  select(Year, est_ssb, om_ssb, peel, sim) %>%
  filter(peel %in% seq(1, 100, 5)) %>%
  group_by(Year, peel) %>%
  median_qi(est_ssb, om_ssb, .width=c(0.50, 0.80)) %>%

  ggplot(aes(x=Year, color=peel))+
    geom_line(aes(y=est_ssb, group=peel))+
    geom_line(aes(y=om_ssb), color="red")+
    scale_y_continuous(limits=c(0, 300))+
    theme_bw()

#' Plot restrospective for F
data %>% as_tibble() %>%
  select(Year, est_F, om_F, peel, sim) %>%
  filter(peel %in% seq(1, 100, 5)) %>%
  group_by(Year, peel) %>%
  median_qi(est_F, om_F, .width=c(0.50, 0.80)) %>%

  ggplot(aes(x=Year, color=peel))+
    geom_line(aes(y=est_F, group=peel))+
    geom_line(aes(y=om_F), color="red")+
