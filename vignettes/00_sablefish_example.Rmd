...
OM3 projects future recriutment as occurring in distinct regimes: first a "low" regime for 20 years, followed by a "high" regime for 5 years. The regimes each have a uniquely parameterized Beverton-Holt stock recruitment relationships that differ with respect to the $R_0$ and $S_0$ paramerers.
...
OM4 assumes that recruitment crashes for the first 20 years of the projection period, while the recruitment for the remainder of the projection period is historcially resamples as in OM1.
```{r, om5, eval=FALSE}
...
MP1 is the NPFMC Tier 3 HCR that is currently used to manage sablefish in Alaska. It is a threshold HCR that uses a target reference point of $SPR_{40\%}$. The HCR function (`tier3`) outputs in units of fihsing mortality (F), as specified by "units". No harvest cap or stability constraints are applied. 
```{r, mp1, eval=FALSE}
mp_f40 <- mp_base
mp_f40$name <- "F40"
