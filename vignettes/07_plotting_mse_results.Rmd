...
Helper functions are available to faciliate simple and consistent visualization of MSE output data. All visualizations are created with `ggplot2` and interacting visualization packages (e.g. `ggdist`, `ggridges`), allowing for them to be easily customized. By default, the "color" aesthetic corresponds to different management procedures, and plots are facetted by operating model.
