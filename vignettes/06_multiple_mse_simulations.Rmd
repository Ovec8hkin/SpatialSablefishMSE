...
To use `run_mse_multiple` users need only provide a list of OM objects, a list of MP/HCR objects, and a list (or vector) of random seeds. The function creates a __factorial grid of combinations of OMs and MPs__ and serially runs `run_mse_parallel` for each combination. Additional parameters required by `run_mse` (such as `nyears`) also need to be provided.
...
run_mse_multiple(oms, mps, seeds, ...)
