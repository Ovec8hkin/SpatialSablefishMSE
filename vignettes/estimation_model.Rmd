...
The estimation method (EM) used by the `SablefishMSE` package is an adapted version of the TMB model built by Craig Marsh and available as part of the [`SpatialSablefishAssessment` package](https://github.com/Craig44/SpatialSablefishAssessment). It is an single-region, two fleet, two survey, age-sex structured assessment model, that closesly mimics the dynamics of the operational Alaska sablefish assessment, build and run annually by Dr. Dan Goethel of NOAA Alaska Fisheries Science Center. The EM diverges from the `SpatialSablefishAssessment` model and the operational model in two key ways:
