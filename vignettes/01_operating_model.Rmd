...
The `SpatialSablefishMSE` package makes use of the `afscOM` package for defining the operating model (OM) component of the MSE simulation loop. As such, defining OMs to use within the MSE framework is identical to the process of defining a model with `afscOM`.
...
- `movement`: Movement matrix `[nregions, nregions, nyears, nages, nsexes]`

For additional information on building the demographic arrays, see the [`afscOM` documentation](https://benwilliams-noaa.github.io/afscOM/articles/specifying_demographic_parameters.html).
...
To simulate observations from the OM, `om$model_options$simulate_observations = TRUE` should be specified. Additionally, a list of observation parameters also needs to be defined. Required observation parameters include:
...
Each list item should be of dimensions `[1, nfleets+surveys]`, as observation processes must be defined for all fleets. 

### The Full Operating Model Object
A fully parameterized OM object, `sable_om`, is available through the `SpatialSablefishMSE` package. This OM object contains a full set of demographic parameter matrices, observation process parameters, and an array of initial numbers-at-age. Demographic parameters for this bassed OM do not vary over space, but a 
movement matrix, as estimation by Cheng et al. 2025, is implemented to simulate spatial dynamics.

```{r, sable_om, eval=FALSE}
# sable_om <- readRDS(file.path(here::here(), "data", "spatial_sablefish_om.RDS"))

# subset_dem_params(sable_om$dem_params, 55, 1, drop=FALSE)
