...
Type objective_function<Type>::operator() () {
  ...
  DATA_SCALAR(sigma_R_early);                       // standard deviation for recruitment;
  DATA_SCALAR(sigma_R_early_end);                   // year to stop using early period sigmaR
  DATA_INTEGER(bias_ramp);                          // whether to turn on bias ramp or not;
  DATA_INTEGER(b_year_start_idx);                       // year index to start bias ramp adjustment
  DATA_INTEGER(b_year_end_idx);                         // year index to stop bias ramp adjustment
  DATA_SCALAR(bmax);                                // maximum bias adjustment
  DATA_SCALAR(ba50);                                // age-50 offset for ascending/descending limbs of bias ramp
  DATA_SCALAR(end_yr_rec_est_idx);                  // last year to estimate rec devs for
  ...
  DATA_IVECTOR(ll_catchatage_indicator);            // dim: n_regions x n_years.  1 = calculate catch at age in this year and region, 0 = don't calculate catch at age
  DATA_ARRAY(obs_ll_catchatage);                   // Longline fishery composition observations dim = 2*n_ages x n_regions x n_years. NOTE: male first then female
  ...
  array<Type> pred_ll_catchatage(obs_ll_catchatage.dim); // Sex disaggregated predicted catch at age
  DATA_SCALAR(loglik_wgt_ll_catchatage);
  ...
  DATA_IVECTOR(srv_dom_ll_age_indicator);                   // dim: n_regions x n_years x n_surveys.  1 = calculate catch at age in this year and region, 0 = don't calculate catch at age
  DATA_ARRAY(obs_srv_dom_ll_age);                   // Longline domestic survey composition observations dim = 2*n_ages x n_regions x n_years x n_surveys. NOTE: male first then female
  DATA_ARRAY_INDICATOR(keep_srv_dom_ll_age_comp, obs_srv_dom_ll_age); // Used for OSA residuals, when not using the multinomial likelihood
  ...
  array<Type> pred_srv_dom_ll_age(obs_srv_dom_ll_age.dim); // Sex disaggregated predicted catch at age
  DATA_SCALAR(loglik_wgt_srv_dom_ll_age);
  ...
  DATA_IVECTOR(srv_nmfs_trwl_age_indicator);                   // dim: n_regions x n_years x n_surveys.  1 = calculate catch at age in this year and region, 0 = don't calculate catch at age
  DATA_ARRAY(obs_srv_nmfs_trwl_age);                   // Longline domestic survey composition observations dim = 2*n_ages x n_regions x n_years x n_surveys. NOTE: male first then female
  DATA_ARRAY_INDICATOR(keep_srv_nmfs_trwl_age_comp, obs_srv_nmfs_trwl_age); // Used for OSA residuals, when not using the multinomial likelihood
  ...
  array<Type> pred_srv_nmfs_trwl_age(obs_srv_nmfs_trwl_age.dim); // Sex disaggregated predicted catch at age
  ...
  vector<Type> b(n_years);          // Recruitment bias adjustment
  ...
  Type sigma_R_early_sq = sigma_R_early * sigma_R_early;
  ...
  /*
   * Compute bias ramp from Methot and Taylor, 2011
   */
  Type b_yr_end_idx = n_years - ba50;
  for(int bias_ramp_year = 0; bias_ramp_year < n_years; ++bias_ramp_year){
    if(bias_ramp == 1){
      if(bias_ramp_year < b_year_start_idx || bias_ramp_year == end_yr_rec_est_idx){
        b(bias_ramp_year) = 0.0;
      }else if(bias_ramp_year >= b_year_start_idx && bias_ramp_year < b_year_end_idx){
        // b(bias_ramp_year) = bmax*(((bias_ramp_year-b_year_start_idx)/(b_year_end_idx-b_year_start_idx)));
        b(bias_ramp_year) = bmax*(1-((bias_ramp_year-b_year_start_idx)/(b_year_end_idx-b_year_start_idx)));
      }else if(bias_ramp_year >= b_year_end_idx && bias_ramp_year < b_yr_end_idx){
        b(bias_ramp_year) = bmax;
      }else if(bias_ramp_year >= b_yr_end_idx && bias_ramp_year < end_yr_rec_est_idx){
        // b(bias_ramp_year) = bmax*(1-(bias_ramp_year - b_yr_end_idx)/(end_yr_rec_est_idx-b_yr_end_idx));
        b(bias_ramp_year) = bmax*(1-(bias_ramp_year - b_yr_end_idx)/(end_yr_rec_est_idx-b_yr_end_idx));
      }
    }else{
      b(bias_ramp_year) = 1.0;
    }
  }
  ...
  Type sigRsq = 0;
  if(bias_ramp == 1){
    sigRsq = sigma_R_early_sq;
  }else{
    sigRsq = sigma_R_sq;
  }

  init_natage_m(0) = exp(ln_mean_rec + ln_rec_dev(0) - sigma_R_early_sq/2.0)/2.0; //mean_rec * exp((sigma_R*sigma_R)/2)/2;
  init_natage_f(0) = init_natage_m(0);
  ...
  for(age_ndx = 1; age_ndx < n_ages; age_ndx++) {
    ...
    if(age_ndx <= n_init_rec_devs) {
      // include recruitment variation in intial age-comp
      init_natage_f(age_ndx) = exp(ln_mean_rec - (M(age_ndx - 1, 0) + init_F_hist * sel_ll_f(age_ndx, 0)) * age_ndx + ln_init_rec_dev(age_ndx - 1) - sigma_R_early_sq/2) / 2;
      init_natage_m(age_ndx) = exp(ln_mean_rec - (M(age_ndx - 1, 0) + init_F_hist * sel_ll_m(age_ndx, 0)) * age_ndx + ln_init_rec_dev(age_ndx - 1) - sigma_R_early_sq/2) / 2;
    } else {
      // assume the last initial age deviation for these
      init_natage_f(age_ndx) = exp(ln_mean_rec - (M(age_ndx - 1, 0) + init_F_hist * sel_ll_f(age_ndx, 0)) * age_ndx + ln_init_rec_dev(ln_init_rec_dev.size() - 1) - sigma_R_early_sq/2) / 2;
      init_natage_m(age_ndx) = exp(ln_mean_rec - (M(age_ndx - 1, 0) + init_F_hist * sel_ll_m(age_ndx, 0)) * age_ndx + ln_init_rec_dev(ln_init_rec_dev.size() - 1) - sigma_R_early_sq/2) / 2;
    }
  }
  // plus group
  ...
  for(year_ndx = 0; year_ndx < n_years; ++year_ndx) {
    ...
    Type sigRsq = 0;
    if(bias_ramp == 1 && year_ndx < sigma_R_early_end){
      sigRsq = sigma_R_early_sq;
    }else{
      sigRsq = sigma_R_sq;
    }
  ...
  vector<Type> numbers_at_age_and_sex(2*n_ages);
  vector<Type> temp_observed_age_and_sex(2*n_ages);
  ...
  for(year_ndx = 0; year_ndx < n_years; ++year_ndx) {
    ...
    if(srv_dom_ll_age_indicator(year_ndx) == 1) {
      // Get catch at age for males and account for ageing error
      temp_numbers_at_age = natage_m.col(year_ndx).vec() * sel_srv_dom_ll_m.col(srv_dom_ll_sel_by_year_indicator(year_ndx)).vec() * S_m_mid.col(year_ndx).vec();
      temp_numbers_at_age_after_ageing_error = (temp_numbers_at_age.matrix().transpose()) * ageing_error_matrix;
      numbers_at_age_and_sex.segment(0,n_ages) = temp_numbers_at_age_after_ageing_error;
      // Now Get catch at age for females and account for ageing error
      temp_numbers_at_age = natage_f.col(year_ndx).vec() * sel_srv_dom_ll_f.col(srv_dom_ll_sel_by_year_indicator(year_ndx)).vec() * S_f_mid.col(year_ndx).vec();
      temp_numbers_at_age_after_ageing_error = (temp_numbers_at_age.matrix().transpose()) * ageing_error_matrix;
      numbers_at_age_and_sex.segment(n_ages,n_ages) = temp_numbers_at_age_after_ageing_error;
      // do posfun so no zero proportions
      for(int i = 0; i < numbers_at_age_and_sex.size(); ++i)
      ...
      if(srv_dom_ll_age_comp_likelihood == 0) {
        ...
        // }
      } else if (srv_dom_ll_age_comp_likelihood == 1) {
        Type N_input = sum(temp_observed_age_and_sex);
        temp_observed_age_and_sex /= N_input;
        nll(6) -= ddirichletmulti(temp_observed_age_and_sex, numbers_at_age_and_sex, N_input, theta_fixed_catchatage, 1);
        // SIMULATE {
    ...
    if(srv_nmfs_trwl_bio_indicator(year_ndx) == 1) {
      ...
      if(srv_nmfs_trwl_bio_likelihood == 0) {
        ...
        Type cv  = se_nmfs_trwl_bio(srv_nmfs_trwl_bio_ndx)/obs_nmfs_trwl_bio(srv_nmfs_trwl_bio_ndx);
        nll(17) -= dlnorm(obs_nmfs_trwl_bio(srv_nmfs_trwl_bio_ndx), log(pred_nmfs_trwl_bio(srv_nmfs_trwl_bio_ndx)) - 0.5 * cv * cv, cv, true);
        // Simulate lognormal observations
        SIMULATE {
  ...
  for(year_ndx = 0; year_ndx < n_init_rec_devs; ++year_ndx)
    nll(22) += square(ln_init_rec_dev(year_ndx)/sigma_R_early) + b(year_ndx)*log(sigma_R_early);
  // for(year_ndx = 0; year_ndx < n_init_rec_devs; ++year_ndx)
  //   nll(22) += square(ln_init_rec_dev(year_ndx) - sigma_R_early_sq / 2.)/(2.* sigma_R_early_sq);
  for(year_ndx = 0; year_ndx < ln_rec_dev.size(); ++year_ndx){
    ...
    nll(22) += square(ln_rec_dev(year_ndx)/sigR)+b(year_ndx)*log(sigR);

    // if(bias_ramp == 1){
    //   nll(22) += square(ln_rec_dev(year_ndx) - b(year_ndx)* sigRsq / 2.)/(2.* sigRsq);
    //   // nll(22) += square(ln_rec_dev(year_ndx))
    // }else{
    //   nll(22) += square(ln_rec_dev(year_ndx) - sigRsq / 2.)/(2.* sigRsq);
    // }
    
  }
  nll(22) += (ln_rec_dev.size() + n_init_rec_devs) * log(sigRsq);
  // F-dev penalty
